...
proof-cache key, so a cache hit is only valid when the assumed contract still
holds for the current callers.
...
Any contract that is not provably implied by callers in the slice is recorded
in `assumed_contracts` on the verdict, so the labeled soundness gate can
distinguish "proved under verified contract" from "proved under hand-asserted
contract".
