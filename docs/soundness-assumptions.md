...
## Precision + Reach (Phase 6)

- **`triage` (6.1) — triage reorders/defers, never refutes.** `surface/specmine/triage.py` scores outliers for plausibility and reorders dispatch (and, under `--budget`, defers low-ranked ones). A `deferred_by_budget` disposition is NOT a verdict — it is re-runnable. With no budget cap the confirmed-bug set is order-independent (zero loss, by construction); under a budget cap the soundness obligation is only "real bugs rank within the cap", which is a *precision/recall* property of the LLM scorer, never a soundness one. The sound checker (Phase 5.3 CBMC) keeps final-verdict authority — triage cannot turn a real bug into a non-bug.

- **`mlta` (6.2) — default Stage A is UNCHANGED; precision mode is opt-in under a documented completeness assumption.** `surface/indirect_calls.py` does NOT modify `surface/reachability.py`. The Phase-1.2 keep-set (all-address-taken over-approximation, 22.05% pruning on netfilter) remains the default, so the Juliet soundness gate (`missed_bug_count = 0`) is untouched. The MLTA *precision mode* (38.24% pruning) resolves each `recv->field(...)` to the functions assigned to that `(type, field)` (or field-only when the receiver type isn't locally visible), harvested from static initializers AND runtime `recv->field = func` assignments. Its soundness rests on the **MLTA completeness assumption**: every indirect-call target is assigned to its field via a syntactic assignment the scanner sees. The residual that this assumption covers (and that strict-sound mode would force back to all-address-taken): targets stored through opaque means (struct `memcpy`, a helper that takes a fn-ptr arg and stores it), and bare `(*fp)(...)` calls with no field name (netfilter: 101 such callsites). This is the same *class* of documented unsoundness the Stage-A `indirect calls / function pointers degrade precision` entry already records — MLTA narrows it; it does not eliminate it. Full LLVM-bitcode TypeDive/DeepType (with escape analysis) is the 6.2.x hook that would close the residual.

- **`mlta` (6.2) — reverse-resolution is fully sound (proposer-side only).** The reverse map (function → `(type, field)` slots → indirect callers via `->field(`) is consumed by Phase 5.2's one-hop establishment check, which is entirely proposer-side: it only changes which outliers are deprioritized, never the final verdict (the sound checker decides). So using MLTA to make indirectly-dispatched callees' callers visible (e.g. `ctnetlink_glue_attach_expect` ← `nfqnl_ct_parse` via `nfnl_ct_hook.attach_expect`) introduces ZERO unsoundness regardless of the completeness assumption — at worst it mis-deprioritizes a lead, which the budget-uncapped verifier still checks.

## Lock-order mining (Phase 7 / 6.x — `surface/specmine/lock_order.py`)

- **`lock-order` — proposer; lockdep is the verdict authority for this class.** Lock-order mining extracts the dominant lock-acquire order from the source (per-function held-stack nesting → ordered pairs → weighted lock-order graph) and flags cycles / minority-weight inversions as *leads*. It does NOT decide a deadlock exists. CBMC/symbolic execution cannot practically prove a lock-ordering deadlock from source, so for this bug class the §8 "sound checker" is **lockdep at runtime** — a lead is confirmed only by a lockdep splat under (directed, Phase 6.3) fuzzing, exactly as the kernelctf-latest hunt confirms Candidate A. The miner is the static proposer that tells the fuzzer *where to aim*.

- **`lock-order` — class-merge over-approximation can create FALSE cycles (a lead-side precision cost, never a missed real order).** Lock-class identity is the canonicalized acquire argument with the receiver before the first `->` dropped (`&mm->mmap_lock` → `mmap_lock`, `&cpuctx->ctx.mutex` → `ctx.mutex`). This merges distinct *instances* of the same field into one class (lockdep does the same — it keys on lock *class*). It can also over-merge two genuinely-different locks that happen to share a field-tail (e.g. two unrelated `->lock` fields both canonicalize to `lock`), producing a spurious cycle. That is extra leads to triage, not a missed ordering. The `--min-dominant-weight` filter (default 2) suppresses the most common artifact — 1-vs-1 cycles (suspicion-0.50 floor), which are almost always a macro-hidden acquire or a branch-insensitive false nesting rather than a real inversion.

- **`lock-order` — known extraction limits (soft, documented).** (a) **Macro-hidden acquires**: `atomic_dec_and_mutex_lock(...)`, `scoped_guard(...)`, `guard(mutex)(...)`, `NF_HOOK(...)` are not unfolded — their lock acquire is missed (same class of limit as 5.1's macro guards). (b) **Branch-insensitivity**: the held-stack is not reset across mutually-exclusive branches, so a lock taken in one branch and another in a different branch (without an intervening release the regex sees) can appear falsely nested — the same branch-local over-release issue documented for the 5.1 extractor. Both bound *what cycles we can see / how noisy the leads are*, never the soundness of the downstream lockdep adjudication. (c) **Scope-bounded**: a cross-subsystem cycle (like Candidate A: `cpuctx_mutex → pmus_lock → cpu_hotplug_lock → … → mmap_lock`) is only visible when all participating subsystems are in scope — a single-subsystem run (e.g. `kernel/events/` alone) sees only its slice of the graph. Whole-kernel scope is the scaling step.
