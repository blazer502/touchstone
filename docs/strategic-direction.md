...
### 산출물 B — Cex-backed PoC (input + 경로 + assertion)
...
- **이걸로 할 수 있는 것**:
  - 책임공개 (responsible disclosure)에 "여기 cex가 있다" 첨부.
  - 회귀 테스트 자동 생성 (cex → unit test).
...
- **무엇**: 한 코드베이스에 대해 cex / safe-proof / 가정-기반-증명을 모두
...
- **무엇**: 현재 패러다임 ("LLM proposes, verifier checks") 을 뒤집어
  *verifier가 cex를 생성하고 LLM이 그걸 소비*해서 다음 artifact를 만듦.
...
- **LLM-only 흉내 불가 이유**: cex를 만들 verifier가 없음. LLM이 자기
  hallucination에 대한 ground truth를 갖지 못함.
- **이걸로 할 수 있는 것**:
  - PoC 생성: cex bytes를 LLM이 받아 자연스러운 입력 형태로 변환 (binary
    blob → structured file).
  - 회귀 테스트 생성: cex → 사람 가독성 있는 testcase.
  - 패치 제안: cex → "여기서 NULL이 가능, 이 가드 추가" 형태로 LLM이 제안.
...
| 책임공개 절차 | cex-backed PoC + 회귀 테스트 자동 생성 (B + E) |
...
**P1. Cex-as-artifact 표준화**
- 현재: CBMC unsafe verdict는 trace에 cex를 적지만 우리 시스템은 그걸
  발견-수준 정보로만 사용.
- 변경: cex를 1급 산출물로 격상. 스키마 (입력 변수 할당 + 경로 + violated
  property + sanitizer 매핑) 표준화. 모든 oracle (Tier 1/2/3) 가 같은
  스키마로 cex 출력.
- 효과: CyberGym에 byte stream으로 변환 가능 + 동시에 disclosure에 첨부
  가능 + 회귀 테스트로 자동 변환 가능. **하나의 cex, 여러 산출물.**
...
- CyberGym 만 보지 말고: Magma, Juliet, kernel CVE feed, OpenSSL, libxml2.
  각 코드베이스에 대해 surface-reduction + sound-pruning + verified-cex 통계를
  publish.
...
CyberGym은 **산출물 B (cex-backed PoC)** 와 **산출물 E (cex-driven LLM)**
의 *데모 케이스*다. 점수는 부산물이지 목적이 아님.

- CyberGym 1507 / 0.33 % 점수 → 우리 산출물 B의 *byte projection only*
  점수. cex의 다른 차원 (assertion, path, sanitizer mapping)은 leaderboard
  는 보지 않지만 우리 산출물에는 살아 있음.
- 진짜 데모: "여기 5개 confirmed task에 대해 우리는 cex + path + violated
...
|---|---|---|---|---|
| P1 cex-as-artifact 표준 스키마 | B, E | 큼 (다회용 산출물) | 1-2일 | 없음 |
...
```
schemas/cex.py                     # 산출물 B의 1급 스키마
  ├─ InputAssignment   (변수 → 값)
  ├─ ExecutionPath     (taken branches)
  ├─ ViolatedProperty  (assertion / sanitizer)
  ├─ SoundnessNote     (이 cex가 어떤 가정 위에 있나)
  └─ Provenance        (어느 engine, 어느 버전, 어느 contract)

# Tier 1/2/3 driver가 모두 같은 Cex 객체를 emit
oracle/tier1_fuzz/verdict.py:  Tier1Verdict → Cex(...)
oracle/tier2_symbolic/verdict.py: ditto
oracle/tier3_bmc/verdict.py:  ditto

# Adapter: byte stream으로 환원하는 helper
schemas/cex.py: Cex.to_bytes() → bytes  (CyberGym에 제출용)
schemas/cex.py: Cex.to_regression_test() → str  (회귀 테스트 생성)
schemas/cex.py: Cex.to_disclosure_blob() → dict (responsible disclosure용)
```
